...
Iteration 1 — DONE: Telegram MVP end-to-end.

Iteration 2 — DONE: Telegram subscriptions, noise control, source graph and digest v0.

Iteration 3 — DONE: source adapters (RSS/Atom, GitHub public, Telegram public, link aggregator), source kind resolver, CLI backup/audit, operational diagnostics, optional n8n outbound router contract.

Полный runtime smoke с реальными Telegram/changedetection.io сервисами требует настроенных env secrets и запущенного self-hosted `changedetection.io`: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_ADMIN_IDS`, `TELEGRAM_ALLOWED_USER_IDS`, `TELEGRAM_ALERT_CHAT_ID`, `CHANGEDETECTION_API_KEY`, `SCOUTBOT_WEBHOOK_SECRET`, `SCOUTBOT_WEBHOOK_URL`.

Текущий MVP поток после Iteration 3:

> Указать публичный URL в Telegram → определить source kind → сохранить target в SQLite → adapter discovery/source graph/degraded artifacts → auto-queue supported links → синхронизировать watches в changedetection.io → принять webhook → сохранить signal → classifier/category/priority → отправить alert подписчикам и optional global sink → при необходимости отметить как noise → собрать artifacts/digest/audit/backup.
...
Текущая схема после Iteration 3:
...
Telegram
  ...
  → SQLite
  → bounded discovery/adapters
  → source graph/degraded artifacts
  → changedetection.io REST API
  → changedetection.io watches/diff
  → ScoutBot webhook
  → signals/classification/artifacts
  → Telegram alerts/subscribers
  → backup/audit/export artifacts
```
...
- noise control;
- classifier categories and priority;
- source graph and degraded artifacts;
- digest v0.
...
- source kind resolver;
- RSS/Atom adapter;
- GitHub public repo/releases/changelog adapter;
- public Telegram webpage adapter;
- link aggregator parser v0;
- adapter target kinds through Telegram `/add`;
- adapter target kinds through seed import/export;
- sync support for adapter target kinds;
- SQLite backup command;
- audit summary command;
- operational diagnostics/docs alignment;
- optional n8n outbound router config remains disabled by default.

## Слои проекта

- **core** — запуск, settings validation, logging, paths, CLI dispatch;
- **SQLite state** — runtime source of truth для workspaces, projects, targets, target links, watches, signals, Telegram subscribers, audit log;
...
- **discovery** — source kind resolver, bounded adapters, public links, RSS/Atom, GitHub public, Telegram public, link aggregators, sitemap/common paths/social links;
- **operations** — backup/audit/export/import CLI для operator workflow;
...
- backup SQLite в `storage/backups/<backup_id>/`;
- audit summary в `storage/runs/<run_id>/audit_summary.json`;
...
- `./start.sh audit`;
- Telegram commands;
- source kind resolver и bounded discovery/adapters для RSS/GitHub/public Telegram/link aggregators;
...
- классифицировать signal по category/priority;
- отправить alert подписчикам и optional global sink в Telegram;
- при необходимости отметить signal как noise;
- собрать digest/artifacts;
...
1. проверяет Telegram allowed user allowlist;
2. валидирует URL;
3. нормализует URL;
...
## Target не обязательно сайт после Iteration 3
...
| `careers` | вакансии |
| `rss` | RSS/Atom feed |
| `github` | GitHub org/general public page |
| `github_repo` | GitHub public repo |
| `github_releases` | GitHub public releases page |
| `github_changelog` | GitHub public changelog candidate |
| `telegram` | Telegram URL, включая unsupported/degraded cases |
| `telegram_public` | публичная Telegram web page через `t.me/s/<channel>` |
| `social_profile` | X/LinkedIn/YouTube/etc. public profile page where available |
| `link_aggregator` | Linktree/Beacons/generic public link page |
| `custom` | ручной источник |

## Telegram UX

Реализовано к завершению Iteration 2:
...
- `/me`;
- `/subscribers`;
- inline keyboard UX (pause, resume, delete, mark as noise);
- Telegram alerts;
- multi-recipient delivery (active subscribers + optional global sink).
- `Mark as noise` из alert работает через signal callback.
...
Требует `TELEGRAM_BOT_TOKEN`. `TELEGRAM_ADMIN_IDS` и `TELEGRAM_ALLOWED_USER_IDS` определяют access model.
...
Собирает ручной digest по signals и отправляет его через общий multi-recipient delivery path.

```bash
./start.sh backup
```

Создаёт воспроизводимый SQLite backup:
...
Пишет compact operational summary:
...
signals:
  ...
  noise:
    ...
    ignore_selectors: ["nav", "footer", ".cookie", ".subscribe"]
...
`Mark as noise` уже реализован:

- signal получает `category=noise`;
- priority становится `low`;
- target получает ignore marker/rule в `ignore_text_json`;
- пишется `noise_update.json`;
- repeated noise/dedupe suppresses repeated signal.
...
- allowed users могут использовать `/start`, `/help`, `/subscribe`, `/unsubscribe`, `/me`, `/add`, `/projects`, `/targets`, `/check`;
- admin-only: `/pause`, `/resume`, `/delete`, `/subscribers`, destructive target callbacks;
- URL валидировать;
...
### Iteration 2 — Signal quality and discovery hardening — DONE
...
После Iteration 3 гарантируется flow:
...
→ Telegram alert to subscribers / optional global sink
→ Mark as noise
→ digest / backup / audit / artifacts
```
...
ScoutBot after Iteration 3 — это:

```text
Telegram-first MVP + SQLite + source kind resolver + bounded source adapters + changedetection.io REST boundary + subscriptions + multi-recipient alerts + noise control + digest + backup/audit operations v0
```
...
- Telegram UX, webhook, discovery, subscriptions, multi-recipient alerts, noise control, digest, source adapters, backup and audit — implemented through Iteration 3;
