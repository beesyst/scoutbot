...
Текущий статус: Iteration 2 DONE — добавлены Telegram subscriptions (`/subscribe`, `/unsubscribe`, `/me`, `/subscribers`), multi-recipient alerts, `TELEGRAM_ALLOWED_USER_IDS`, noise control (`Mark as noise`), улучшенный classifier с Iteration 2 категориями, priority scoring, source graph artifact с explainable статусами, degraded source detection, CLI digest (`./start.sh digest`), исправлена утечка Telegram token в логах.
...
Текущий MVP поток после Iteration 2:

> Указать публичный URL в Telegram → сохранить target в SQLite → bounded discovery и source graph/degraded artifacts → синхронизировать watches в changedetection.io → принять webhook → сохранить signal → classifier/category/priority → отправить alert подписчикам и optional global sink → при необходимости отметить как noise → собрать artifacts/digest.
...
Текущая схема после Iteration 2:
...
## Target не обязательно сайт после Iteration 2
...
После Iteration 2 гарантируется flow:
...
ScoutBot after Iteration 2 — это:

```text
Telegram-first MVP + SQLite + bounded discovery + changedetection.io REST boundary + subscriptions + multi-recipient alerts + noise control + digest v0
```
...
- Telegram UX, webhook, discovery, subscriptions, multi-recipient alerts, noise control and digest — implemented through Iteration 2;
